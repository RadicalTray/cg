...
void update(Resources* resources, const float dt_s, std::uniform_real_distribution<>& dis, std::mt19937& gen, const Config config) {
    ...
    for (size_t i = 0; i < 4 * rain_count; i += 4) {
        ...
        if (quad.v[0].pos.y <= -1.0f) {
            const float splash_radius = 0.02f;
            const int splash_particles = 5;

            for (int j = 0; j < splash_particles; ++j) {
                float angle = dis(gen) * 3.14159f;
                float dx = cos(angle) * splash_radius;
                float dy = sin(angle) * splash_radius;

                RainVertex v = quad.v[0];
                v.pos.x += dx;
                v.pos.y = -1.0f + dy;

                resources->rain_vertices[i] = v;
            }

            quad.setPosY(0, 1.0f + (dis(gen) + 1.0f) + quad.height());
